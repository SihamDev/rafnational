...
function badgeSales(s: string) {
  ...
  return cn('rounded-full px-2.5 py-0.5 text-[11px] font-semibold', map[s] ?? 'bg-gray-100 text-gray-600')
}
...
export default async function AgentDashboard() {
  ...
  let q2 = supabase.from('leads').select('id', { count: 'exact', head: true }).eq('sales_workflow_status', 'new')
  let q3 = supabase.from('leads').select('id', { count: 'exact', head: true }).gte('next_followup_at', now).lte('next_followup_at', todayEndStr)
  let q4 = supabase.from('leads').select('id', { count: 'exact', head: true }).not('next_followup_at', 'is', null).lt('next_followup_at', now)
  let q5 = supabase.from('leads').select('id', { count: 'exact', head: true }).eq('sales_workflow_status', 'converted')
  let q6 = supabase.from('leads').select('id,first_name,family_name,phone_number,city,qualification_status,sales_workflow_status,next_followup_at,visit_source_raw,created_at').not('next_followup_at', 'is', null).lte('next_followup_at', todayEndStr).order('next_followup_at', { ascending: true }).limit(20)
  ...
  return (
    <>
      ...
      <main className="flex-1 p-5 md:p-6 space-y-5" dir="rtl">

        {/* ── Greeting banner ── */}
        <div className="rounded-2xl bg-navy-900 px-6 py-5 text-white overflow-hidden relative">
          <div className="pointer-events-none absolute -top-10 right-8 h-40 w-40 rounded-full bg-brand/10 blur-[60px]" />
          <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-brand/60">
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
          {([
            { label: 'إجمالي المسندة',    value: totalAssigned ?? 0, icon: Briefcase,    cls: 'bg-white',       href: '/admin/leads' },
            { label: 'جدد لم يُتصل بهم',  value: newLeads ?? 0,      icon: Zap,          cls: 'bg-amber-50',    href: '/admin/leads?sales=new' },
            { label: 'متابعة اليوم',       value: dueToday ?? 0,      icon: Clock,        cls: 'bg-blue-50',     href: '#due' },
            { label: 'متأخرة',             value: overdue ?? 0,        icon: AlertCircle,  cls: overdue ? 'bg-rose-50' : 'bg-white', href: '#due' },
            { label: 'تحويلات',            value: converted ?? 0,      icon: CheckCircle2, cls: 'bg-emerald-50',  href: '/admin/leads?sales=converted' },
          ] as const).map((kpi) => {
            const Icon = kpi.icon
            return (
              <Link
        ...
        <div className="grid gap-4 lg:grid-cols-5">
          ...
          {/* ── Due & overdue leads ── */}
          <div id="due" className="lg:col-span-3 rounded-2xl border border-black/[0.07] bg-white shadow-sm overflow-hidden">
            <div className="flex items-center justify-between border-b border-black/[0.05] px-5 py-4">
              <div className="flex items-center gap-2">
                <Clock size={15} className="text-brand" />
                <span className="font-bold text-navy-900 text-sm">مواعيد المتابعة</span>
              </div>
              <Link href="/admin/leads" className="text-[11px] font-semibold text-brand hover:underline">
                عرض الكل
              </Link>
            </div>
              ...
              <div className="divide-y divide-black/[0.04]">
                {dueLeads.map((lead) => {
                  ...
                  return (
                    <div
                      key={lead.id}
                      className={cn('flex items-center gap-3 px-5 py-3.5', isOverdue && 'bg-rose-50/50')}
                    >
                      <div className={cn('shrink-0 h-2 w-2 rounded-full', isOverdue ? 'bg-rose-500' : 'bg-amber-400')} />

                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <Link
                            href="/admin/leads"
                            className="font-semibold text-[13px] text-navy-900 hover:text-brand truncate"
                          >
                            {name || '—'}
                          </Link>
                          <span className={badgeSales(lead.sales_workflow_status)}>
                            {SALES_STATUS_LABELS[lead.sales_workflow_status as keyof typeof SALES_STATUS_LABELS] ??
                        ...
                        <div className="flex items-center gap-3 mt-0.5 text-[11px] text-gray-400">
                          ...
                          {dueDate && (
                            <span className={cn('font-mono', isOverdue && 'text-rose-500 font-semibold')}>
                              {isOverdue ? '⚠ متأخر — ' : ''}
                              {dueDate}
                            </span>
                          )}
                      ...
                      {wa && (
                        <a
                          ...
                          className="flex items-center gap-1 rounded-lg bg-emerald-50 px-2.5 py-1.5 text-[11px] font-semibold text-emerald-700 hover:bg-emerald-100 transition-colors shrink-0"
                        >
          ...
          {/* ── Pipeline ── */}
          <div className="lg:col-span-2 rounded-2xl border border-black/[0.07] bg-white shadow-sm overflow-hidden">
            <div className="flex items-center gap-2 border-b border-black/[0.05] px-5 py-4">
              <TrendingUp size={15} className="text-brand" />
              <span className="font-bold text-navy-900 text-sm">مسار العملاء</span>
            </div>
                ...
                return (
                  <div key={key}>
                    ...
                    <div className="h-1.5 w-full rounded-full bg-gray-100 overflow-hidden">
                      <div
                        className={cn('h-1.5 rounded-full transition-all duration-500', PIPE_COLORS[key] ?? 'bg-gray-400')}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </div>
            ...
            {/* Quick links */}
            <div className="border-t border-black/[0.05] px-5 py-3 flex flex-wrap gap-2">
              <Link
