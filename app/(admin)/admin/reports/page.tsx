...
export default async function ReportsPage() {
  ...
  for (const r of salaryRows ?? []) {
    const s =
      ((r as { salary_range_raw: string | null }).salary_range_raw ?? '').trim() || 'غير محدد'
  ...
  for (const r of amountRows ?? []) {
    const a =
      ((r as { requested_amount_raw: string | null }).requested_amount_raw ?? '').trim() ||
      'غير محدد'
    ...
    <>
      ...
      <main className="flex-1 space-y-5 p-5 md:p-6" dir="rtl">
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          {/* Source breakdown */}
          <ReportCard
            title="توزيع مصادر الزيارة"
            icon={Megaphone}
            items={sourceData}
            max={barMax(sourceData)}
            color="bg-brand"
          />
          ...
          {/* Salary breakdown */}
          <ReportCard
            title="توزيع شرائح الراتب"
            icon={TrendingUp}
            items={salaryData}
            max={barMax(salaryData)}
            color="bg-emerald-500"
          />

          {/* Requested amount */}
          <ReportCard
            title="المبالغ المطلوبة"
            icon={TrendingUp}
            items={amountData}
            max={barMax(amountData)}
            color="bg-purple-500"
          />
        </div>
      </main>
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white/70 bg-white/90 shadow-[0_2px_20px_-6px_rgba(14,26,51,0.10)] backdrop-blur-sm">
      <div className="relative border-b border-gray-100/80 px-5 py-4">
        <div className="via-brand/40 absolute start-0 end-0 top-0 h-[1.5px] bg-gradient-to-r from-transparent to-transparent" />
        <div className="flex items-center gap-2.5">
          <div className="bg-ink flex h-7 w-7 items-center justify-center rounded-lg">
            ...
            return (
              <div key={label} className="py-3">
                <div className="mb-1.5 flex items-center justify-between">
                  <span className="text-ink max-w-[65%] truncate text-[12px] font-semibold">
                    {label}
                  </span>
